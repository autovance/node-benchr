...
module.exports = class Runner extends EventEmitter {
  ...
  _run(file) {
    ...
    // Import test file.
    let testFn = require(path.resolve(process.cwd(), file));

    // Allow for ES6 default exports.
    if (typeof testFn === 'object' && testFn !== null && 'default' in testFn) {
      testFn = testFn.default;
    }
  ...
  addSuite(name, opts, fn) {
    ...
    // Create a new suite
    let suite  = this.suites[this.suites.length] = new Benchmark.Suite(name, opts);
    suite.opts = opts;
